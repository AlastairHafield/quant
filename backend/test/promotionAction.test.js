...
test('describePromotionAction: an unapproved gate result produces no action, with reasons carried through', () => {
  const result = describePromotionAction('gex-breakout', { approved: false, reasons: ['walk-forward Sharpe too low'] });
  assert.equal(result.action, 'none');
...
test('executePromotionAction: an unapproved gate result executes nothing', async () => {
  const result = await executePromotionAction('gex-breakout', { approved: false });
  assert.equal(result.action, 'none');
...
test('executePromotionAction: throws if PROMOTION_GATE_API_KEY is unset', async () => {
  ...
  try {
    await assert.rejects(() => executePromotionAction('gex-breakout', { approved: true }));
  } finally {
...
test('executePromotionAction: PATCHes the Heroku config-vars API and reports executed:true on success', async () => {
  await withMockedFetch(
    { ok: true, json: async () => ({ STRATEGY_OF_EXECUTION_ENABLED: 'true' }) },
    async () => {
      const result = await executePromotionAction('gex-breakout', { approved: true });
      assert.equal(result.executed, true);
      assert.deepEqual(result.data, { STRATEGY_OF_EXECUTION_ENABLED: 'true' });
    },
  );
});
