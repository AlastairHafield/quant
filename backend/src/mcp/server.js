...
export function createBackendMcpServer() {
  ...
  server.registerTool('ledger_trades', {
    ...
    description: 'Raw trade docs, optionally filtered by system (Mongo db name: gex_breakout), dayKey, closedFrom/closedTo range (bare YYYY-MM-DD or full ISO timestamps), or strategy (a trade doc\'s own `strategy` field — "OF" = Order Flow Bot signal trades, "reconciled" = untracked shared-account fills NOT from OF signals, "B" = retired legacy strategy; omit to get all of them pooled together, which is almost never what you want when comparing against an OF-specific backtest).',
    inputSchema: {
  ...
  server.registerTool('reconciliation_run', {
    ...
    description: 'Compares the strategy\'s actual recent live trades against a backtest\'s predicted stats over the same window. backtestStats is a backtest run\'s metrics.full or .oos (from orderflow_backtest_run above). Pass `strategy` to restrict live trades to one `strategy`-field value (see ledger_trades) — required for a meaningful OF-backtest comparison, since unfiltered live trades pool OF signal trades together with unrelated "reconciled"/"B" activity.',
    inputSchema: {
  ...
  server.registerTool('reconciliation_shadow_days', {
    ...
    description: 'Builds promotionGate\'s shadowDays array in one call. Each day\'s comparison is CUMULATIVE (day 1..N, not day N alone) — this strategy trades too infrequently for a single day to hit the 5-trade minimum to be comparable. See reconciliation.js\'s buildShadowDayReports. Pass `strategy` to restrict to one `strategy`-field value (see ledger_trades / reconciliation_run).',
    inputSchema: {
