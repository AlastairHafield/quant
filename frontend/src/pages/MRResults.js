...
import { getMRRuns, getMRTrades, getMRSweep } from '../api';
...
const SIGNAL_LABELS = {
  VWAP_FADE: 'VWAP Fade', RSI_BB_FADE: 'RSI/BB Fade', GAP_FADE: 'Gap Fade', PDL_FADE: 'PD Extreme Fade',
};

export default function MRResults({ initialRunId, initialSweepId }) {
  ...
  async function selectRun(id, runList = null) {
    ...
    try {
      ...
      setSelRun(run || { id });
      if (run?.sweep_id) {
        const sw = await getMRSweep(run.sweep_id);
        setSweepRuns(sw.data || []);
      } else {
        setSweepRuns([]);
      }
      const res = await getMRTrades(id);
      setTrades(res.data || []);
  ...
  async function openSweep(sweepId) {
    ...
    try {
      await loadRuns();
      const sw = await getMRSweep(sweepId);
      const data = sw.data || [];
      setSweepRuns(data);
      if (data.length > 0) {
        setSelRun(data[0]);
        const res = await getMRTrades(data[0].id);
        setTrades(res.data || []);
      }
    } catch (e) {} finally { setLoading(false); }
  }
  ...
  return (
    <div>
      <p className="page-title">Mean-Reversion Results</p>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {runs.map(run => (
              <button
                ...
                style={{ fontSize: 10 }}
              >
                #{run.id} {run.symbol} {SIGNAL_LABELS[run.signal_type] || run.signal_type} ({run.timeframe})
      ...
      {loading ? (
        ...
        <div className="empty">No results yet. Run a backtest in the MR Lab first.</div>
      ) : (
        <>
          {/* Sweep leaderboard */}
          {sweepRuns.length > 1 && (
            <div className="card">
              ...
              <div style={{ color: 'var(--text3)', fontSize: 11, fontFamily: 'var(--mono)', marginTop: 6 }}>
                ...
                A top row whose neighbours are all red is curve-fit — don't trust it.
              </div>
            </div>
          )}
          ...
          {m && (
            <div className="card">
              <p className="card-title">
                {selectedRun.symbol} · {SIGNAL_LABELS[selectedRun.signal_type] || selectedRun.signal_type} · {selectedRun.timeframe}
          ...
          {m && (
            <div className="two-col">
              <BreakdownCard title="By Daily Trend Regime" data={m.byTrend}
                hint="If FLAT days are green and UP/DOWN days red, turn on the trend/ADX filter." />
              <BreakdownCard title="By Entry Hour (NY)" data={m.byHour}
                hint="MR usually works best mid-session. Trim the session window to the green hours." />
            </div>
          )}
          {m && (
            <div className="two-col">
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>Exit</th><th>Result</th><th>Bars</th><th>Regime</th><th>Sample</th><th>Return %</th><th>P&amp;L</th>
                  </tr>
                </thead>
                <tbody>
                  {trades.map(t => (
