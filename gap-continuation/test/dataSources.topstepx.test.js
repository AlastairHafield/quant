...
// This adapter's TradeBarAggregator also tracks `open` (unlike the other two
// bots') — the gap-detection strategy needs a real session-open print, not
// just OHLC-minus-open like ORB/order-flow do.
test("TradeBarAggregator: builds OHLC (incl. open) and buy/sell volume within a single minute", () => {
  const bars = [];
  ...
  agg.flush();
  assert.deepEqual(bars, [{ open: 5500, high: 5502, low: 5498, close: 5498, buyVolume: 15, sellVolume: 8 }]);
});
...
test("TradeBarAggregator: flushes automatically when a trade crosses into the next minute", () => {
  ...
  assert.deepEqual(bars[0], { open: 5500, high: 5505, low: 5500, close: 5505, buyVolume: 10, sellVolume: 4 });

  agg.flush();
  assert.equal(bars.length, 2);
  assert.deepEqual(bars[1], { open: 5510, high: 5510, low: 5510, close: 5510, buyVolume: 6, sellVolume: 0 });
});
...
test("buildBracketOrderRequest: long order — stop ticks negative (below entry), target ticks positive (above)", () => {
  ...
  assert.deepEqual(req.takeProfitBracket, { ticks: 12, type: ORDER_TYPE.LIMIT }); // 3pts above entry (1:1 R:R)
});
...
test("buildBracketOrderRequest: short order — stop ticks positive (above entry), target ticks negative (below)", () => {
  const req = buildBracketOrderRequest({
    ...
    targetPrice: 7457,
    tickSize: 0.25,
    customTag: "gapcont-short-124",
  });
  assert.equal(req.side, ORDER_SIDE.SELL);
  assert.deepEqual(req.stopLossBracket, { ticks: 12, type: ORDER_TYPE.STOP });
  assert.deepEqual(req.takeProfitBracket, { ticks: -12, type: ORDER_TYPE.LIMIT });
});

const accounts = [
  { id: 1, name: "50KTC-V2-DLL-416538-32353936", balance: 50000, canTrade: true, isVisible: true },
  { id: 2, name: "COMBINE_OTHER", balance: 47950, canTrade: false, isVisible: true },
];
...
test("selectAccount: a nameHint disambiguates among multiple tradable accounts", () => {
  const multi = [...accounts, { id: 3, name: "PRAC-V2-416538-98727790", balance: 150000, canTrade: true, isVisible: true }];
  assert.equal(selectAccount(multi, "PRAC-V2").id, 3);
});

test("selectAccount: throws with the candidate list when multiple tradable accounts exist and no hint disambiguates", () => {
  const multi = [...accounts, { id: 3, name: "PRAC-V2-416538-98727790", balance: 150000, canTrade: true, isVisible: true }];
  assert.throws(() => selectAccount(multi), /50KTC-V2-DLL-416538-32353936.*PRAC-V2-416538-98727790|PRAC-V2-416538-98727790.*50KTC-V2-DLL-416538-32353936/);
});
