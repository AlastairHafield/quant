...
test("buildTradeTakenEmbed: shows the real target price (this strategy always has one, unlike ride-to-EOD bots)", () => {
  const embed = buildTradeTakenEmbed({
    ...
    stopPrice: 7447,
    targetPrice: 7477,
    reasonFields: [["Gap %", "0.62"], ["ADX", "31.4"]],
    orderId: 9101,
  });
  const e = embed.embeds[0];
  assert.match(e.description, /7477/);
...
test("buildTradeTakenEmbed: no fixed target shows 'ride to EOD' instead of a price", () => {
  ...
  assert.match(embed.embeds[0].description, /ride to EOD/);
});
...
test("flushLogBufferToDiscord: skips when there are no rows or no webhook configured", async () => {
  ...
  assert.deepEqual(await flushLogBufferToDiscord("https://discord.test/webhook", [], "2026-07-27", "scheduled", fakeFetch), { skipped: true });
  assert.deepEqual(await flushLogBufferToDiscord(null, [{ ts: "t" }], "2026-07-27", "scheduled", fakeFetch), { skipped: true });
  assert.equal(called, false);
});
...
test("flushLogBufferToDiscord: posts a multipart form with the JSONL attachment on success", async () => {
  ...
  const result = await flushLogBufferToDiscord("https://discord.test/webhook", [{ ts: "t1" }], "2026-07-27", "scheduled", fakeFetch);
