...
export class Worker {
  ...
  // re-snapshots "now" as the day's start, the same known limitation
  // reconcileUntrackedPosition already has for stop/target recovery.
  ...
  async tripRiskHalt(reason) {
    ...
    console.error(`RISK HALT (gap-continuation): ${reason}`);
    await postDiscordEmbed(
      CONFIG.discord.webhook,
      buildSignalEmbed({
        title: "🛑 RISK HALT — Gap Continuation",
        description: `All new entries blocked for the rest of the day: **${reason}**`,
        color: 0xe74c3c,
        fields: [["Day", this.riskDayKey]],
        footerText: `gap-continuation · ${new Date().toISOString()}`,
      })
  ...
  handleSignal(result) {
    // Shared with Mechanical ORB on the same real account/MES contract — and,
    // dormant but still wired to the same account, gex-breakout's "default"
    // role (accountRoleFor in gex-breakout/src/worker.js; only its Order Flow
    // Bot is active today, routed to a separate practice account, but
    // "default" still polls this same real account and would trade it if
